...
MAX_ITEMS_ON_FIRST_RUN = 10  # 최초 실행 시 한꺼번에 보낼 최대 개수 (나머지는 기록만 하고 스킵)
...
def run_once():
    ...
    # 최초 실행 시에는 전체 상품이 다 "신규"로 잡혀서 한꺼번에 쏟아짐 -> 상한선 적용
    if is_first_run and len(new_items) > MAX_ITEMS_ON_FIRST_RUN:
        print(f"[최초 실행] {len(new_items)}건 중 최근 {MAX_ITEMS_ON_FIRST_RUN}건만 전송, 나머지는 전송 기록에만 남김")
        to_send = new_items[:MAX_ITEMS_ON_FIRST_RUN]
        # 전송 안 한 것들도 "이미 본 것"으로 기록해서 다음 회차에 재알림 안 되게 처리
        sent_ids.update(item["id"] for item in new_items)
    else:
        to_send = new_items
        sent_ids.update(item["id"] for item in new_items)
